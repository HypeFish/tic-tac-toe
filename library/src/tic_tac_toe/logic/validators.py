...
def validate_winner(grid: Grid, starting_mark: Mark, winner: Mark | None) -> None:
    if winner == "X":
        if starting_mark == "X" and grid.x_count <= grid.o_count:
            raise InvalidGameState("Wrong number of Xs")
        elif starting_mark == "O" and grid.x_count != grid.o_count:
            raise InvalidGameState("Wrong number of Xs")
    elif winner == "O":
        if starting_mark == "O" and grid.o_count <= grid.x_count:
            raise InvalidGameState("Wrong number of Os")
        elif starting_mark == "X" and grid.o_count != grid.x_count:
            raise InvalidGameState("Wrong number of Os")
